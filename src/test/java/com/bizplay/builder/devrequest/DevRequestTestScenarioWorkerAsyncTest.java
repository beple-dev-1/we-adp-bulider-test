...
class DevRequestTestScenarioWorkerAsyncTest {
    ...
    /**
     * ⛔ <b>이미 개발에 넘긴 요청서에는 TC 를 새로 만들지 않는다</b> (2026-09-23). 넘긴 뒤 스냅샷에 TC 가
     * 생기면 개발이 받은 꾸러미에는 없는 번호로 받기가 판정한다 — 「보낸 적 없는 TC」가 거꾸로 선다.
     */
    @Test
    void 이미_넘긴_요청서에는_만들지_않는다() {
        TaskExecutor aiExecutor = mock(TaskExecutor.class);
        DevRequestTestScenarioWorker worker = new DevRequestTestScenarioWorker(
                mock(DevelopmentRequestMapper.class), mock(FrdMapper.class),
                mock(DevRequestTestScenarioReader.class), mock(ClaudeCredentialRunner.class),
                mock(BuilderProperties.class), mock(ObjectMapper.class), aiExecutor);
        DevelopmentRequest request = mock(DevelopmentRequest.class);
        DevelopmentRequestContent content = mock(DevelopmentRequestContent.class);
        given(request.id()).willReturn("0000010");
        given(request.deliveryState()).willReturn(DevelopmentRequest.DeliveryState.SENT);
        given(content.acceptanceCriteria()).willReturn(List.of(
                new DevelopmentRequestContent.Note("ACCEPTANCE_CRITERION", "완료 조건")));

        assertThat(worker.requestIfMissing(request, content)).isFalse();
